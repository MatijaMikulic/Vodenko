...
namespace MessageBroker.Common.Producer
{
    ...
    public class RabbitMqProducerConsumer:IProducerConsumer
    {
        ...
        /// </summary>
        public async Task OpenCommunication(string clientName = "Client")
        {
            ...
            while (true)
            {
                try
                {
                    _connection = _rabbitMqService.CreateConnection(clientName);
        ...
        {
            //string routingKey = _routingKeyQueueMap.FirstOrDefault(x => x.Value == queue).Key;
